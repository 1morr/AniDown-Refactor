...
class TestTorrentCompletionHandlerContext:
    """Tests for context passing in TorrentCompletionHandler."""

    @pytest.fixture
    def mock_download_repo(self):
        """Create mock download repository."""
        ...
        repo = MagicMock()
        repo.get_by_hash.return_value = DownloadRecord(
            hash=TorrentHash('abc123def456789012345678901234567890abc'),
            anime_id=1,
            original_filename='Test Anime',
            anime_title='Test Anime',
            subtitle_group='Test Group',
            season=1,
            download_directory='/downloads/Test',
            status=DownloadStatus.DOWNLOADING,
            download_method=DownloadMethod.RSS_AI,
            download_time=datetime.now(timezone.utc),
            requires_tvdb=True  # TVDB required
        )
        return repo

    @pytest.fixture
    def mock_anime_repo(self):
        """Create mock anime repository."""
        from src.core.domain.entities import AnimeInfo
        from src.core.domain.value_objects import (
            AnimeTitle, SubtitleGroup, SeasonInfo, Category, MediaType
        )
        from datetime import datetime, timezone

        repo = MagicMock()
        repo.get_by_id.return_value = AnimeInfo(
            id=1,
            title=AnimeTitle(original='Test', short='Test', full='Test Anime'),
            subtitle_group=SubtitleGroup(name='Test Group'),
            season=SeasonInfo(number=1, category=Category.TV),
            category=Category.TV,
            media_type=MediaType.ANIME,
            tvdb_id=123456,  # Has TVDB ID
            created_at=datetime.now(timezone.utc)
        )
        return repo

    def test_tvdb_fetched_when_requires_tvdb_and_has_id(
        self,
        mock_download_repo,
        mock_anime_repo
    ):
        """
        Test that TVDB data is fetched when:
        1. requires_tvdb = 1 (True)
        2. anime_info has TVDB ID

        Expected: TVDB service should be called with the TVDB ID
        """
        from src.services.download.torrent_completion_handler import TorrentCompletionHandler

        mock_download_client = MagicMock()
        mock_download_client.get_torrent_files.return_value = [
            {'name': 'Episode 01.mp4', 'size': 1000000000}
        ]

        mock_file_renamer = MagicMock()
        mock_file_renamer.classify_files.return_value = (
            [MagicMock(name='Episode 01.mp4')],  # videos
            []  # subtitles
        )
        mock_file_renamer.generate_mapping.return_value = RenameResult(
            main_files={'Episode 01.mp4': 'Test - S01E01.mp4'},
            method='ai'
        )

        mock_hardlink_service = MagicMock()
        mock_hardlink_service.build_target_directory.return_value = '/library/Test/Season 1'

        # The handler should fetch TVDB data when conditions are met
        handler = TorrentCompletionHandler(
            anime_repo=mock_anime_repo,
            download_repo=mock_download_repo,
            download_client=mock_download_client,
            hardlink_service=mock_hardlink_service,
            file_renamer=mock_file_renamer
        )

        # This test verifies the expectation that TVDB should be fetched
        # The actual implementation depends on how TorrentCompletionHandler
        # integrates with TVDB service

        # Get the anime info
        anime_info = mock_anime_repo.get_by_id(1)

        # Verify anime has TVDB ID
        assert anime_info.tvdb_id == 123456

        # Verify download requires TVDB
        download = mock_download_repo.get_by_hash('abc123')
        assert download.requires_tvdb is True
