...
# =============================================================================
# Anime Detail Page APIs
# =============================================================================


@anime_bp.route('/anime/<int:anime_id>/detail')
def anime_detail_page(anime_id: int):
    """动漫详情页面"""
    return render_template('anime_detail.html', anime_id=anime_id)


@anime_bp.route('/api/anime/<int:anime_id>/detail')
@inject
@handle_api_errors
def api_get_anime_detail(
    anime_id: int,
    anime_service: AnimeService = Provide[Container.anime_service]
):
    """获取动漫详情API（包含源文件和硬链接）"""
    ...
    logger.api_request(f'获取动漫详情（含文件）- ID:{anime_id}')

    result = anime_service.get_anime_detail_with_files(anime_id)

    if 'error' in result:
        logger.api_error_msg(f'/api/anime/{anime_id}/detail', result['error'])
        return APIResponse.not_found(result['error'])

    logger.api_success(
        f'/api/anime/{anime_id}/detail',
        f'源文件:{len(result.get("source_files", []))}, '
        f'硬链接:{len(result.get("hardlinks", []))}'
    )
    return APIResponse.success(**result)


@anime_bp.route('/api/anime/<int:anime_id>/files/rename', methods=['POST'])
@inject
@handle_api_errors
@validate_json()
def api_rename_files(
    ...
    data = request.get_json()
    mappings = data.get('files', [])

    if not mappings:
        return APIResponse.bad_request('请提供要重命名的文件列表')

    logger.api_request(f'重命名文件 - 动漫ID:{anime_id}, 数量:{len(mappings)}')

    result = anime_service.rename_files(anime_id, mappings)

    if result.get('success'):
        logger.api_success(
            f'/api/anime/{anime_id}/files/rename',
            f'成功:{len(result.get("renamed", []))}'
        )
        return APIResponse.success(**result)
    else:
        return APIResponse.internal_error('重命名失败')


@anime_bp.route('/api/anime/<int:anime_id>/files/delete', methods=['POST'])
@inject
@handle_api_errors
@validate_json()
def api_delete_files(
    ...
    delete_hardlinks = data.get('delete_hardlinks', True)
    delete_source = data.get('delete_source', False)

    if not file_ids:
        return APIResponse.bad_request('请提供要删除的文件ID列表')

    logger.api_request(
        f'删除文件 - 动漫ID:{anime_id}, 数量:{len(file_ids)}, '
        f'删除硬链接:{delete_hardlinks}, 删除源文件:{delete_source}'
    )

    result = anime_service.delete_selected_files(
        anime_id=anime_id,
        file_ids=file_ids,
        delete_hardlinks=delete_hardlinks,
    ...
    if result.get('success'):
        logger.api_success(
            f'/api/anime/{anime_id}/files/delete',
            f'硬链接:{result.get("hardlinks_deleted", 0)}, '
            f'源文件:{result.get("source_deleted", 0)}'
        )
        return APIResponse.success(**result)
    else:
...
@anime_bp.route('/api/anime/<int:anime_id>/files/move', methods=['POST'])
@inject
@handle_api_errors
@validate_json()
def api_move_files(
    ...
    if not file_ids:
        return APIResponse.bad_request('请提供要移动的文件ID列表')

    if not destination:
        return APIResponse.bad_request('请提供目标文件夹名称')

    logger.api_request(
        f'移动文件 - 动漫ID:{anime_id}, 数量:{len(file_ids)}, '
        f'目标:{destination}'
    )

    result = anime_service.move_hardlink_files(
        anime_id=anime_id,
        file_ids=file_ids,
    ...
    if result.get('success'):
        logger.api_success(
            f'/api/anime/{anime_id}/files/move',
            f'成功:{len(result.get("moved", []))}'
        )
        return APIResponse.success(**result)
    else:
        return APIResponse.internal_error('移动失败')


@anime_bp.route('/api/anime/<int:anime_id>/files/ai-rename', methods=['POST'])
@inject
@handle_api_errors
@validate_json()
def api_ai_rename_files(
    anime_id: int,
    anime_service: AnimeService = Provide[Container.anime_service],
    rename_service: RenameService = Provide[Container.rename_service]
):
    """发送文件到AI重命名API"""
    if anime_id < 1:
        return APIResponse.bad_request('动漫ID必须大于0')
    ...
    if not source_files:
        return APIResponse.bad_request('请提供要处理的源文件列表')

    logger.api_request(f'AI重命名 - 动漫ID:{anime_id}, 文件数:{len(source_files)}')

    result = anime_service.send_files_to_ai_rename(
        anime_id=anime_id,
    ...
    if 'error' in result:
        logger.api_error_msg(f'/api/anime/{anime_id}/files/ai-rename', result['error'])
        return APIResponse.internal_error(result['error'])

    logger.api_success(
        f'/api/anime/{anime_id}/files/ai-rename',
        f'映射数:{len(result.get("mappings", []))}'
    )
    return APIResponse.success(**result)


@anime_bp.route('/api/anime/<int:anime_id>/files/apply-ai-rename', methods=['POST'])
@inject
@handle_api_errors
@validate_json()
def api_apply_ai_rename(
    anime_id: int,
    anime_service: AnimeService = Provide[Container.anime_service]
):
    """应用AI重命名结果API"""
    ...
    if not mappings:
        return APIResponse.bad_request('请提供要应用的映射列表')

    logger.api_request(
        f'应用AI结果 - 动漫ID:{anime_id}, 映射数:{len(mappings)}, '
        f'替换全部:{replace_all}'
    )

    result = anime_service.apply_ai_rename_results(
        anime_id=anime_id,
        mappings=mappings,
    ...
    if result.get('success'):
        logger.api_success(
            f'/api/anime/{anime_id}/files/apply-ai-rename',
            f'创建:{len(result.get("created", []))}, '
            f'替换:{len(result.get("replaced", []))}'
        )
        return APIResponse.success(**result)
    else:
        return APIResponse.internal_error('应用AI结果失败')
